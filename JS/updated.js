...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Menu navigation - updated to handle both menu-item and menu-item-admin classes
    const menuItems = document.querySelectorAll('.menu-item, .menu-item-admin');
    ...
    menuItems.forEach(item => {
        ...
        item.addEventListener('click', function(e) {
            ...
            // Update menu active state - include both classes in selection
            document.querySelectorAll('.menu-item, .menu-item-admin').forEach(mi => mi.classList.remove('active'));
...
function loadUsers() {
    ...
    users.forEach(user => {
        ...
        // Change the action button based on whether the user is an admin
        let actionButton;
        
        if (user.isAdmin) {
            if (user.username === currentAdmin.username) {
                // Current admin's account
                actionButton = `<button class="action-btn view-btn" data-username="${user.username}" title="You cannot edit your own admin account">
                    <i class="fas fa-lock"></i>
                </button>`;
            } else {
                // Other admin accounts
                actionButton = `<button class="action-btn view-btn" data-username="${user.username}" title="Admin accounts cannot be edited">
                    <i class="fas fa-eye"></i>
                </button>`;
            }
        } else {
            // Regular users
            actionButton = `<button class="action-btn edit-btn" data-username="${user.username}">
    ...
    // Add click event for view buttons (admin users)
    ...
    viewButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (username === currentAdmin.username) {
                ...
                Swal.fire({
                    title: "Restriction",
                    text: "You cannot edit your own admin account for security reasons.",
                    icon: "warning",
                });
            } else {
                // Other admin accounts
                openViewUserModal(username);
            }
        });
    });
}

// Open edit user modal - modified function
function openEditUserModal(username) {
    ...
    // Don't allow editing admin users, including current admin
    if (user.isAdmin) {
        if (user.username === currentAdmin.username) {
            // The admin is trying to edit their own account
            Swal.fire({
                title: "Restriction",
                text: "You cannot edit your own admin account for security reasons.",
                icon: "warning",
            ...
            // The admin is trying to edit another admin account
            Swal.fire({
                title: "Restriction",
                text: "Admin accounts cannot be edited or deleted.",
                icon: "warning",
              });
        }
        return;
...
// Open view-only user modal for admin users
function openViewUserModal(username) {
    ...
    // Change modal title
    modalTitle.textContent = 'View Admin User';
    ...
    document.getElementById('editConfirmPassword').disabled = true;
    editProfilePicInput.disabled = true;
    
    // Hide profile pic overlay
    const profilePicOverlay = editProfilePicPreview.querySelector('.profile-pic-overlay');
    ...
    submitBtn.onclick = function(e) {
        ...
        // Reset modal title and form fields
        ...
        editProfilePicInput.disabled = false;
...
function setupUserEditing() {
    ...
    editUserForm.addEventListener('submit', function(e) {
        ...
        // Check if user is trying to edit their own admin account
        const originalUsername = document.getElementById('editUserId').value;
        if (originalUsername === currentAdmin.username && currentAdmin.isAdmin) {
            const errorMsg = document.getElementById('edit-error');
            errorMsg.textContent = 'You cannot edit your own admin account!';
            errorMsg.style.display = 'block';
            return;
        }
        
        // Check if this is just a close action for admin view
        const submitBtn = this.querySelector('.submit-btn-update');
        if (submitBtn && submitBtn.textContent === 'Close') {
            document.getElementById('editUserModal').classList.remove('show');
            
            // Reset the form elements to default state
            ...
            document.getElementById('editPassword').disabled = false;
            document.getElementById('editConfirmPassword').disabled = false;
            document.getElementById('editProfilePicInput').disabled = false;
            submitBtn.textContent = 'Update User';
        ...
        // Check if user is admin - shouldn't reach here normally but as an additional protection
        if (currentUser.isAdmin) {
            errorMsg.textContent = 'Admin accounts cannot be edited!';
            errorMsg.style.display = 'block';
            return;
        }
        ...
        // Create alert message for each type of change
        let changes = [];
        if (newUsername !== originalUsername) changes.push(`Username changed to: ${newUsername}`);
        if (email !== currentUser.email) changes.push(`Email changed to: ${email}`);
        if (password) changes.push('Password has been changed');
        if (finalEditProfilePic) changes.push('Profile picture has been updated');

        // Alert for admin
        Swal.fire({
            title: "User Updated",
            html: `User "${originalUsername}" has been updated successfully!<br><br>` + 
    ...
    deleteUserBtn.addEventListener('click', function(e) {
        ...
        // Check if user is admin - additional protection
        const users = JSON.parse(localStorage.getItem('users')) || [];
        const userToCheck = users.find(u => u.username === userToDelete);
    ...
    confirmDeleteBtn.addEventListener('click', function() {
        ...
        // Check one more time if user is admin
        const userToCheck = users.find(u => u.username === userToDelete);
        if (userToCheck && userToCheck.isAdmin) {
            Swal.fire({
                title: "Cannot Delete Admin",
                text: "Admin accounts cannot be deleted! Remove admin privileges first.",
                icon: "error",
            });
            confirmDeleteModal.classList.remove('show');
            userToDelete = null;
        ...
        Swal.fire({
            title: "User Deleted",
            text: `User "${userToDelete}" has been successfully deleted!`,
            icon: "success",
        });
    ...
    document.getElementById('closeEditModal').addEventListener('click', function() {
        ...
        const modalTitle = document.querySelector('.modal-title');
        if (modalTitle.textContent === 'View Admin User') {
            modalTitle.textContent = 'Edit User';
            document.getElementById('editUsername').disabled = false;
            document.getElementById('editEmail').disabled = false;
            document.getElementById('editPassword').disabled = false;
            document.getElementById('editConfirmPassword').disabled = false;
            document.getElementById('editProfilePicInput').disabled = false;
            // Fix: Use correct class for the submit button
